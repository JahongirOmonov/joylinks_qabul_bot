...
async def received_messages(message: MessageStates, state: FSMContext, bot: Bot):
    ...
    users = RegisteredUsers.objects.all().order_by('id')
    ...
    for i in range(0, len(undelivered_users), max_length):
        ...
        # await bot.send_message(chat_id=7628428313, text=chunk)
        url = f"https://api.telegram.org/bot{API_TOKEN}/sendMessage"
        payload = {"chat_id": 7628428313, "text": chunk}
        requests.post(url, json=payload)
    await state.clear()
    # await bot.send_message(chat_id=7628428313, text=undelivered_users)
...
async def message_result(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        await bot.send_message(chat_id=user.chat_id, text=admins_message, parse_mode="HTML")
        if message.chat.id != 7628428313:
            await bot.send_message(chat_id=7628428313, text=secret, parse_mode="HTML")
...
async def users(message: MessageStates, bot: Bot):
    ...
    # celery qismini op tashadim

    bot = Bot(token=API_TOKEN)
    all_users = RegisteredUsers.objects.all().order_by('id')
...
async def ban_user(message: Message, bot: Bot):
    try:
        ...
        if created:
            local_time = localtime(banned_until)
            await message.reply(
            ...
            await message.reply(
                ...
                f"Yangi ban muddati: {local_time}"
            )
            await bot.send_message(chat_id=profile.chat_id,
                                   text=f"Sizning ban yangilandi.\nSabab: {reason}\nBan muddati: {local_time}\n\n"
                                        f"Adminga shikoyatingizni \"/xabar text\" ko'rinishida qoldirishingiz mumkin!")
        ...
        await message.reply(
            f"Foydalanuvchi @{profile.first_name or profile.username} {ban_time} minutga ban qilindi.\nSabab: {reason}")
...
async def unban_user(message: Message, bot: Bot):
    try:
        ...
        await bot.send_message(chat_id=profile.chat_id,
                               text="Sizning ban olib tashlandi. Endi botdan foydalanishingiz mumkin.")
